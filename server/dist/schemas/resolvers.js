...
const resolvers = {
    ...
    Mutation: {
        ...
        createAdventure: async (_parent, { title, characterName, characterClass }, context) => {
            if (!context.user)
                throw new AuthenticationError('Not logged in');
        ...
        advanceAdventure: async (_parent, { adventureId, choiceText }, context) => {
            if (!context.user)
                throw new AuthenticationError('Not logged in');
      ...
      Please provide the next story node text and choices.`;
            ...
            adventure.currentNode = nextNode._id;
